...
from transformers import AutoConfig, AutoModelForCausalLM, AutoProcessor, AutoTokenizer, HfArgumentParser
try:
...
file_handler = logging.FileHandler("training_multimodal_llamaomni2.log")
...
def ensure_llama_omni2_registered():
    ...
    try:
        ...
        logger.warning(f"⚠️  Failed to import llama_omni2 registry hooks: {err}")
        return False

def load_llama_omni2_model(model_name: str, load_kwargs: Dict, disable_speech_modules: bool = False) -> torch.nn.Module:

    ensure_llama_omni2_registered()
    config = AutoConfig.from_pretrained(model_name, trust_remote_code=True)
    if disable_speech_modules and hasattr(config, "speech_encoder"):

        delattr(config, "speech_encoder")
    if disable_speech_modules and hasattr(config, "speech_generator"):
    ...
    try:
        ...
        logger.warning(f"⚠️  AutoModelForCausalLM load failed: {err}")
...
def ensure_language_model_layers(model, layers):
    ...
    if hasattr(model, "language_model"):
        ...
        if hasattr(lm, "model"):
            if not hasattr(lm.model, "layers"):
                setattr(lm.model, "layers", layers)
        else:
            lm.model = SimpleNamespace(layers=layers)
    else:
...
def resolve_hidden_size(model, layers):
    ...
    try:
        ...
        if hasattr(layer0, "self_attn") and hasattr(layer0.self_attn, "q_proj"):
            return layer0.self_attn.q_proj.weight.shape[-1]
    except Exception:
        pass
    raise ValueError("Unable to resolve hidden_size for LLaMA-Omni2 model.")
...
class ScriptArguments:
    ...
    model_name_or_path: Optional[str] = field(
        default="",
        metadata={"help": "HF repo id or local model directory"},
    )
    ...
    cuda_device: Optional[int] = field(default=0, metadata={"help": "CUDA device index to use for single-GPU training"})
    model_load_mode: Optional[str] = field(default="fp16")
    training_precision: Optional[str] = field(
        default="bf16",
        metadata={"help": "Training precision: auto|fp32|fp16|bf16"},
    )
...
if __name__ == "__main__":
    ...
    if script_args.cuda_device < 0 or script_args.cuda_device >= torch.cuda.device_count():
        raise ValueError(
            f"Invalid cuda_device={script_args.cuda_device}. Available device range: 0-{torch.cuda.device_count() - 1}"
        )
    torch.cuda.set_device(script_args.cuda_device)
    run_device = f"cuda:{script_args.cuda_device}"
    logger.debug(f"🎮 Using CUDA device: {run_device}")
    ...
    model_tag = f"llama_omni2_{modality}"
    vector_tag = f"{script_args.behavior}_{model_tag}"
    ...
    requested_training_precision = (script_args.training_precision or "auto").lower()
    if requested_training_precision not in {"auto", "fp32", "fp16", "bf16"}:
        raise ValueError(
            f"Unsupported training_precision '{requested_training_precision}'. "
            "Valid options: ['auto', 'fp32', 'fp16', 'bf16']"
        )
    logger.debug(
        f"🔧 Resolved load precision: bf16={precision_flags['bf16']}, fp16={precision_flags['fp16']}, "
        f"training_precision={requested_training_precision}"
    )
    if requested_training_precision == "fp32":
        precision_flags["bf16"] = False
        precision_flags["fp16"] = False
    elif requested_training_precision == "fp16":
        precision_flags["bf16"] = False
        precision_flags["fp16"] = True
    elif requested_training_precision == "bf16":
        if not torch.cuda.is_bf16_supported():
            logger.warning("Requested bf16 training but this GPU does not support bf16. Falling back to fp16.")
            precision_flags["bf16"] = False
            precision_flags["fp16"] = True
        else:
            precision_flags["bf16"] = True
    ...
    if modality == "text":
        ...
        processor = AutoProcessor.from_pretrained(script_args.model_name_or_path, trust_remote_code=True)
    ...
    if use_swanlab:
        safe_model_name = re.sub(r"[^0-9A-Za-z_.+-]", "_", script_args.model_name_or_path)
        swanlab.init(
    ...
    logger.debug(f"Loading main model to {run_device}...")
    model = load_llama_omni2_model(
        script_args.model_name_or_path,
        load_kwargs,
        disable_speech_modules=(modality == "text"),
    )
    ...
    logger.debug(f"🔧 Wrapping layer {script_args.layer} with BlockWrapper (path={layers_path})...")
    try:
        target_layer = layers[script_args.layer]
        wrapped_layer = BlockWrapper(target_layer, hidden_size=hidden_size)

        wrapped_layer = wrapped_layer.to(run_device)
        ...
        layers[script_args.layer] = wrapped_layer
        logger.debug(f"✅ Layer {script_args.layer} wrapped successfully")
        logger.debug(f"   Device: {next(wrapped_layer.parameters()).device}")
    except IndexError:
        logger.error(f"Layer {script_args.layer} is out of bounds. Model has {len(layers)} layers.")
        exit(1)
    ...
    target_param_name = f'{layers_path}.{script_args.layer}.vec'
    frozen_count = 0
    trainable_count = 0
    ...
    logger.debug(f"🔧 Single-GPU mode enabled on {run_device}")
