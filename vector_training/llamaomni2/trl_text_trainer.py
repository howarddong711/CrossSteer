...
class CrossSteerTextTrainer(Trainer):
    ...
    def concatenated_inputs(
        ...
        if is_encoder_decoder:
            ...
            concatenated_batch["concatenated_decoder_input_ids"] = torch.cat(
                [
                    self._pad_tokens_to_max_length(
                        processed_batch["chosen_decoder_input_ids"],
                        max_decoder_length,
                        pad_value=self.padding_value,
                    ),
                    self._pad_tokens_to_max_length(
                        processed_batch["rejected_decoder_input_ids"],
                        max_decoder_length,
                        pad_value=self.padding_value,
                    )
                ],
            ...
            concatenated_batch["concatenated_decoder_attention_mask"] = torch.cat(
                [
                    self._pad_tokens_to_max_length(
                        processed_batch["chosen_decoder_attention_mask"],
                        max_decoder_length,
                        pad_value=0,
                    ),
                    self._pad_tokens_to_max_length(
                        processed_batch["rejected_decoder_attention_mask"],
                        max_decoder_length,
                        pad_value=0,
                    )
                ],
    ...
    def concatenated_forward(
        ...
        if "text_input_ids" not in model_kwargs and "kimi" in model_name_lower:
            if self.input_modality == "text":
                model_kwargs["text_input_ids"] = torch.zeros_like(concatenated_batch["concatenated_input_ids"])
            else:
                model_kwargs["text_input_ids"] = concatenated_batch["concatenated_input_ids"]

        runtime_model = model
        if hasattr(self, "accelerator"):
            try:
                runtime_model = self.accelerator.unwrap_model(model)
            except Exception:
                runtime_model = model
        ...
        if isinstance(all_logits, (tuple, list)):
            if len(all_logits) >= 2:
                ...
                raise RuntimeError("Model returned empty logits tuple/list.")
        ...
        if self.aux_loss_enabled:
            return (chosen_logps, rejected_logps, chosen_logits_mean, rejected_logits_mean, outputs.aux_loss)

        return (chosen_logps, rejected_logps, chosen_logits_mean, rejected_logits_mean)
    ...
    def log(self, logs: Dict[str, float], start_time: Optional[float] = None, **kwargs) -> None:
        ...
        for split in ("train", "eval"):
            split_metrics = self._stored_metrics.get(split, {})
            for key, metrics in split_metrics.items():
                if metrics:
        ...
        if swanlab is not None:
            ...
            for key, value in logs.items():
                if isinstance(value, torch.Tensor):
                    if value.numel() == 1:
                        swan_metrics[key] = value.detach().cpu().item()
                elif isinstance(value, (float, int, np.floating, np.integer)):
                    swan_metrics[key] = value
            if swan_metrics:
                swanlab.log(swan_metrics, step=self.state.global_step)

        trainer_log_params = inspect.signature(Trainer.log).parameters
        if "start_time" in trainer_log_params:
