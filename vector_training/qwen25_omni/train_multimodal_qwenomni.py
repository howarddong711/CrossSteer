...
from transformers import AutoProcessor, AutoTokenizer, Qwen2_5OmniForConditionalGeneration, HfArgumentParser
...
file_handler = logging.FileHandler("training_multimodal_qwenomni.log")
...
def patch_qwen2_5_omni_load_speakers():
    ...
    try:
        ...
        logger.warning(f"⚠️  Unable to import qwen2_5_omni modeling for patch: {err}")
        return

    def patched_load_speakers(self, *args, **kwargs):
        logger.debug("⚠️  Skipped load_speakers (PyTorch<2.6 workaround)")
        return None

    try:
        qwen_mod.Qwen2_5OmniForConditionalGeneration.load_speakers = patched_load_speakers
        logger.debug("✅ Patched Qwen2.5-Omni load_speakers")
    except Exception as err:
...
class ScriptArguments:
    ...
    layer: Optional[int] = field(default=13)
...
if __name__ == "__main__":
    ...
    model_tag = f"qwen2_5_omni_{modality}"
    ...
    logger.debug(f"🔧 Precision flags: bf16={precision_flags['bf16']}, fp16={precision_flags['fp16']}")
    ...
    if use_swanlab:
        ...
        safe_model_name = re.sub(r"[^0-9A-Za-z_.+-]", "_", model_name_only)
        safe_model_name = safe_model_name.strip("._-+") or "qwen2_5_omni"
        swanlab.init(
